            ...
            switch (guino_data.cmd) 
            {
                ...
                case guino_addChar:
                    ...
                    {  // crappy hack to compensate for gui   
                        ...
                        ((ofxUIWidgetWithLabel *)guino_items[guino_data.item])->label->label = _name;
...
void testApp::addColumn()
{
    ...
    ofAddListener(gui_column[gui_column.size()-1]->newGUIEvent,this,&testApp::guiEvent);
}
void testApp::guinoClear()
{
    ...
    if(gui_column.size() !=0)
    {
        ...
        {
        ofRemoveListener(gui_column[i]->newGUIEvent, this,&testApp::guiEvent); 
	...
	{			
            ...
            gui1->toggleVisible();
        {
            ...
            if(gui1->isVisible())
            {
                for(int i = 0; i < gui_column.size();i++)
                {
                    gui_column[i]->rect->x = gui_column[i]->rect->x +(length+xInit); 
                }
